...
SimulateStepStep::SimulateStepStep(ApplicationManager* pApp)
     :Action(pApp)
{
	MyCPU = new Variable;
}

void SimulateStepStep::ReadActionParameters()
{
...
void SimulateStepStep::SubSimulate(Statement * StartSt,Statement * EndSt)
{
	...
	{
		Select::UnSelectAll(pManager);
		StartSt->SetSelected(1);
		...
		}else if (dynamic_cast<Condition *>(StartSt))  // A Loop Statement is encountered;
		{
			...
			//Proceed to the next Statment;
			 StartSt->SetSelected(0);
			 StartSt=CurConnetor->getDstStat();
			 
			 SimMessage = MyCPU->AllVar() + " Click to Proceed to Next Statement; ";
			 pManager->pOut->PrintMessage(SimMessage);
			 pManager->pIn->GetPointClicked(p);
